...
async function autoGenSeo(body: any, apiKey: string, googleKey?: string): Promise<{description:string, metaDescription:string, keywords:string[], titleSuffix:string, whyChoose:string[], seoText:string} | null> {
  ...
  try {
    ...
    // description에서 "in [area], Seoul, Seoul" 패턴 방지 지침
    const locationHint = area.toLowerCase() === 'seoul'
      ? 'Location: Seoul'
      : `Location: ${area}, Seoul — write "${area}" or "${area}, Seoul" but NEVER "${area}, Seoul, Seoul"`
...
{"metaDescription":"[max 155 chars, include '${shop.name}' and '${area}']","excerpt":"[2 sentence hook for the blog card]","tags":["${shop.name} review","${area} ${cat}","${cat} Seoul foreigners","Seoul beauty 2026","${shop.name} worth it"]}`
  ...
  try {
    ...
    // 혹시 AI가 생성한 <figure>/<img> 태그 제거 (사진은 블로그 상세 페이지에서 갤러리로 렌더링)
...
<div class="sp-wrap">
  ...
  ${(()=>{
    /* ── Why Choose (AI 생성 bullets) — 2열 카드 그리드 ── */
    ...
    const items = wc.map((b:string, bi:number)=>{
  ...
  ${(()=>{
    ...
    if(shop.category === 'clinic'){
      ...
      return '<div class="sp-seo-block">'+_fbHead
        ...
        +'<p class="sp-seo-p">As a full-service '+areaGn+' dermatology clinic, '+shop.name+' provides a comprehensive range of medical aesthetic treatments popular among foreign patients: '+treatments+'. Korean dermatology clinics like '+shop.name+' use the latest KFDA-approved equipment, offering results that are often 40\u201360% more affordable than equivalent treatments in the US, UK, or Australia.</p>'
        +'<h2 class="sp-seo-h2">Why Foreign Patients Choose '+shop.name+'</h2>'
        +'<p class="sp-seo-p">For foreigners visiting Seoul, finding a clinic with English-speaking staff and transparent fees is the biggest challenge. '+shop.name+' solves this with dedicated English-speaking coordinators, clear pricing, and seamless WhatsApp booking. Whether you are a first-time medical tourist or a returning patient, the team ensures your comfort from initial consultation through aftercare.</p>'
        +'<h2 class="sp-seo-h2">How to Book '+shop.name+' as a Foreigner</h2>'
        +'<p class="sp-seo-p">Booking through Seoul Beauty Trip takes under 2 minutes. Tap the WhatsApp button above, describe your desired treatment, and our English-speaking team will confirm your appointment, explain pricing, and prepare the clinic for your visit. No Korean needed. Same-day and advance bookings available.</p>'
        +'</div>';
    }
      ...
      ${nearbyCrossShops.map((r: any) => {
        ...
        return `<a href="/shop/${r.slug}" style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:12px;text-decoration:none;transition:background .15s" onmouseover="this.style.background='rgba(255,255,255,.06)'" onmouseout="this.style.background='rgba(255,255,255,.03)'" title="${r.name} ${_rCatLabel} in ${_rArea} Seoul">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:10.5px;color:rgba(255,255,255,.45);margin-top:2px"><i class="fas ${_rIcon}" style="color:var(--pk2);font-size:8px;margin-right:3px"></i>${_rCatLabel}${_rRating ? ` · ★${_rRating}` : ''}</div>
          </div>
...
${SB_TRACKER_SCRIPT}
  ...
  if (shopPhotoUrls.length > 0) {
    ...
    if (shopPhotoUrls.length === 1) {
      ...
      // 2장 이상: 가로 스크롤 스트립
      const items = shopPhotoUrls.map((u, i) =>
        `<div class="shop-photo-item"><img src="${u}" alt="${altBase} photo ${i+1}" loading="lazy" onerror="this.parentElement.style.display='none'"></div>`
      ).join('')
...
// GET /api/admin/debug-blog-photos?slug=xxx — 블로그 포토 갤러리 디버그
app.get('/api/admin/debug-blog-photos', async (c) => {
  try {
    ...
    return c.json({
      ...
      p0PhotoName: (() => { const u=String(photoArr[0]||''); const i=u.indexOf('/photos/'); return i<0?'':u.slice(i+8,i+68) })(),
      p1PhotoName: (() => { const u=String(photoArr[1]||''); const i=u.indexOf('/photos/'); return i<0?'':u.slice(i+8,i+68) })(),
